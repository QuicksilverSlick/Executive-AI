/**
 * Chat API Fallback Endpoint
 * Provides voice agent functionality using OpenAI Chat API + TTS
 * 
 * Features:
 * - Chat completion with function calling
 * - Text-to-speech synthesis
 * - Session management
 * - Compatible with voice agent interface
 */
...
async function generateSpeech(
  ...
  const response = await fetch('https://api.openai.com/v1/audio/speech', {
    ...
    body: JSON.stringify({
      model: 'tts-1',
      input: text.substring(0, 4096), // TTS has input limits
      voice,
      response_format: 'mp3',
    }),
  });
 ...
 * Process voice agent requests using Chat API + TTS fallback
 */
...
Current mode: Chat API fallback (standard OpenAI API with TTS)`
    ...
    // Define available functions (simplified for fallback mode)
    const functions = [
      {
    ...
    // Generate audio if requested and we have a text response
...
/**
 * OPTIONS /api/voice-agent/chat-fallback
